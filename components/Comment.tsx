...
const Comments = ({ comment, setComment, addComment, comments, isPostingComment }: IProps) => {
  ...
  return (
    <div className="border-t-2 border-gray-300 pt-4 px-10 mt-4 bg-[#F8F8F8] border-b-2 lg:pb-0 pb-[100px]">
      <div className="overflow-scroll lg:h-[457px]">
        {comments?.length > 0 ? (
          ...
          <NoResults text="Make sure Login Google if want see the comment right here! Be First to do add the comment." />
        )}
      </div>
      {userProfile && (
        <div className="absolute bottom-0 left-0  pb-6 px-2 md:px-10 ">
          <form onSubmit={addComment} className="flex gap-4">
            <input
              ...
              placeholder="Tambahkan Komenya Guys.."
            />
            <button className="text-md text-gray-400 " onClick={addComment}>
              {isPostingComment ? "Mengirim..." : "Comment"}
            </button>
          </form>
        </div>
      )}
